...
In \Cref{fig:PPSQ}, the graph of the querying result, the
...
In our need to understand the use of cache, we need to examine the different
...
performance. But, it does not mean that it is impossible to target the same set in the same slice. Previous work~\cite{EURECOM+4671} showed that the hash function could be reverse engineered, for example, in \Cref{fig:hashfunc}.
...
\section{Cache Template Attacks Paper} \label{sec:Cache Template Attacks Paper}
\begin{itemize}
